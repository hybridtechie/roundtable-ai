...
const SortableQuestion: React.FC<{
  ...
  return (
    <div className="flex items-center justify-between p-2 mb-2 bg-gray-100 rounded">
      <div ref={setNodeRef} style={style} {...attributes} {...listeners} className="flex-grow cursor-move">
        <span>{question.content}</span>
      </div>
      <Button variant="ghost" size="icon" onClick={handleRemoveClick} className="w-8 h-8 ml-2 rounded-full" type="button">
        <X className="w-4 h-4" />
                ...
                <ul className="w-full min-h-[100px] border border-dashed border-gray-300 p-4 rounded-md">
                  {finalQuestions.length === 0 ? (
                    ...
                    finalQuestions.map((question) => (
                      <SortableQuestion key={question.id} question={question} onRemove={removeFromFinalList} />
                    ))
                  )}
                </ul>
              </SortableContext>
          ...
          <div className="w-[70%] space-y-2">
            ...
            <div className="space-y-2 max-h-[300px] overflow-y-auto p-2 border border-gray-200 rounded-md">
              {isQuestionsLoading ? (
                ...
                <div className="py-4 text-center text-gray-400">No AI-generated questions available</div>
              ) : (
